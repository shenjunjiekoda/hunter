...
const char* column_type_str(hunter::sqlite::DbColumnType type) {
    switch (type) {
        using enum hunter::sqlite::DbColumnType;
        case Text:
            return "TEXT";
        case Integer:
            return "INTEGER";
        ...
        case Blob:
            return "BLOB";
        default:
            hunter_unreachable("invalid column type");
    }
}

} // anonymous namespace

namespace hunter::sqlite {
...
void DbConnection::exec_command(const char* cmd) {
    hunter_assert_msg(cmd != nullptr, "cmd is null");
...
void DbConnection::begin_transaction() {
    hunter_assert(this->m_commit_policy == CommitPolicy::Manual);
...
void DbConnection::create_table(
    ...
    hunter_assert_msg(!columns.empty(), "columns is empty");
...
DbOstream::DbOstream(DbConnection& db, llvm::StringRef table_name, int columns)
    ...
    hunter_assert_msg(columns > 0, "invalid number of columns");
...
void DbOstream::add(llvm::StringRef s) {
    hunter_assert(s.size() <= static_cast< std::size_t >(
...
void DbOstream::flush() {
    hunter_assert_msg(this->m_current_column == this->m_columns + 1,
                      "incomplete row");
