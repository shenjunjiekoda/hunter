...
//===------------------------------------------------------------------===//
//
//  This file defines the reporter for hunter diagnostics.
//
//===------------------------------------------------------------------===//
...
void DiagnosticReporter::report(const HunterDiagnostic& diagnostic) {
    using namespace clang;
...
void DiagnosticReporter::apply_fixes() {
    ...
    for (const auto& [file, pair] : m_file_to_build_dir_and_replaces) {
        ...
        if (auto replacements_formatted =
            ...
            hunter_assert_msg(replacements_expected,
