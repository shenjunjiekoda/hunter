...
//===------------------------------------------------------------------===//
//
//  This file implements the diagnostic used by the hunter tool.
//
//===------------------------------------------------------------------===//

#include "analyzer/tooling/diagnostic.hpp"
#include "analyzer/tooling/context.hpp"
// #include "analyzer/tooling/hunter.hpp"
...
struct Less {
    bool operator()(const HunterDiagnostic& lhs,
                    const HunterDiagnostic& rhs) const {
...
HunterDiagnostic::HunterDiagnostic(llvm::StringRef checker,
                                   Level diag_level,
                                   llvm::StringRef build_dir)
    : clang::tooling::Diagnostic(checker, diag_level, build_dir) {}

HunterDiagnosticConsumer::HunterDiagnosticConsumer(HunterContext& context)
    : m_context(context) {}

void HunterDiagnosticConsumer::HandleDiagnostic(
    ...
    if (diag_level == DiagnosticsEngine::Note) {
        hunter_assert_msg(!m_diags.empty(),
        ...
        HunterDiagnostic::Level level = HunterDiagnostic::Warning;
        switch (diag_level) {
            default:
                level = HunterDiagnostic::Warning;
                break;
            case DiagnosticsEngine::Error:
                [[fallthrough]];
            case DiagnosticsEngine::Fatal:
                level = HunterDiagnostic::Error;
                break;
            case DiagnosticsEngine::Remark:
                level = HunterDiagnostic::Remark;
                break;
        }
...
std::vector< HunterDiagnostic > HunterDiagnosticConsumer::take_diags() {
...
void HunterDiagnosticRenderer::emitDiagnosticMessage(
    ...
    auto to_char_range = [this, &loc](const CharSourceRange& range) {
        ...
        hunter_assert(range.isTokenRange());
    ...
    if (level == DiagnosticsEngine::Note) {
        m_diag.Notes.push_back(hunter_msg);
...
void HunterDiagnosticRenderer::emitCodeContext(
    ...
    for (const auto& fix_it : hints) {
        ...
        hunter_assert_msg(char_range.getBegin().isValid() &&
                              char_range.getEnd().isValid(),
                          "Invalid range in the fix-it hint.");
        hunter_assert_msg(char_range.getBegin().isFileID() &&
