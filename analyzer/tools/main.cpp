...
//===------------------------------------------------------------------===//
//
//  This is the main file of the hunter-analyzer tool.
//
//===------------------------------------------------------------------===//
...
analyzer::AnalyzerOptions get_analyzer_options() {
    ...
    return forward_to_hunter_analyzer_options(analyzer_argc,
                                              analyzer_argv.data());
}

std::unique_ptr< HunterOptionsProvider > get_opts_provider() {
    auto opts_provider = std::make_unique< HunterOptionsCommandLineProvider >();
...
std::vector< std::string > get_enabled_checkers() {
    HunterContext context(std::move(get_opts_provider()));
    const HunterASTConsumerFactory factory(context);
...
int main(int argc, const char** argv) {
    ...
    HunterContext ctx(std::move(opts_provider));
    HunterDriver driver(ctx,
    ...
    if (const bool compile_error_found =
            llvm::any_of(diags, [](const auto& diag) {
                return diag.DiagLevel == HunterDiagnostic::Error;
            })) {
